...
func (s *Server) AnthropicMessages(c *gin.Context) {
	...
	// Forward request to provider
	response, err := s.forwardRequest(provider, openaiReq)
	if err != nil {
		c.JSON(http.StatusInternalServerError, ErrorResponse{
			Error: ErrorDetail{
				Message: "Failed to forward request: " + err.Error(),
...
func (s *Server) convertAnthropicToOpenAI(anthropicReq *AnthropicMessagesRequest) *RequestWrapper {
	...
	// We'll handle these parameters in the forwardRequest function
	...
	// We'll handle stop sequences in the forwardRequest function
